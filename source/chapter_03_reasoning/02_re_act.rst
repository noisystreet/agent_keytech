...
.. admonition:: ReAct 的名字为什么这么起？
   :class: story

   作者 Shunyu Yao 在论文中解释："ReAct"这个名字既是对"Reasoning + Acting"
   的缩写，也是在暗指化学中的**反应物** （Reactant）——推理和行动就像
   两种化学物质，单独存在时各自做着简单的事，但放在一起就会产生剧烈的
   **链式反应** ，释放出远超各自之和的能量。

   这个命名很贴切：ReAct 不是把"想"和"做"简单串起来，而是在它们之间创造了
   **反馈回路**——行动的结果修正思考的方向，思考的结论指导下一步的行动。
