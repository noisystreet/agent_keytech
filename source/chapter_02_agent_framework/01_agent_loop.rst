...
.. admonition:: Agent 循环的起源：从强化学习到 LLM
   :class: story

   Agent 的"感知-推理-行动"循环并不是 LLM 时代的发明。它的思想根源
   可以追溯到 1980 年代的**强化学习**：智能体（Agent）观察环境状态 →
   选择行动 → 获得奖励 → 更新策略。这个框架在围棋 AI（AlphaGo）、
   机器人控制、自动驾驶中已经证明了它的威力。

   LLM Agent 的特殊之处在于：传统 RL Agent 的"推理"是一个黑箱
   （神经网络直接输出行动概率），而 LLM Agent 的推理是**可解释的
   自然语言**。你能看到它在想什么，也能告诉它"你想错了，换个方向"。
   这让 LLM Agent 的调试和干预比传统 RL Agent 容易得多。
