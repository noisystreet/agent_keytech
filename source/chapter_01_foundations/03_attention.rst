...
.. admonition:: Attention 的诞生：谷歌翻译的意外产物
   :class: story

   2017 年 Google 团队在《Attention Is All You Need》中提出 Transformer
   时，注意力机制并不是为了"理解语言"而设计的——它是为了解决机器翻译中
   **长句子翻译质量差** 的问题。当时的 Seq2Seq 模型在翻译超过 30 个词的
   句子时准确率急剧下降，因为编码器的最后一层必须把整个句子的信息
   压缩成一个固定长度的向量。

   Attention 的解决思路优雅得令人惊叹：**不要压缩，直接查**——解码时
   每一步都能"回看"输入序列的每一个位置。这个今天看起来理所当然的想法，
   在 2017 年是一种颠覆性的创新。它的第一作者 Vaswani 后来回忆说：
   "我们花了几个月验证这个想法，因为我们自己都不太敢相信它真的有效。"
