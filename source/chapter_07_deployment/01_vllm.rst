...
.. admonition:: PagedAttention 的灵感：操作系统的虚拟内存
   :class: story

   vLLM 的作者 Kwanghyun Park 在 2022 年注意到 Transformer 推理中
   KV Cache 的管理方式和操作系统处理物理内存的方式惊人地相似——
   都是碎片化分配、预分配浪费、空间利用率低的问题。于是他借鉴了
   操作系统的**分页存储**方案。

   这个看似简单的跨界借鉴，带来了推理框架领域近十年最大的突破。
   今天几乎所有的 LLM 推理优化框架（SGLang、TensorRT-LLM 等）
   都采用了类似的分页思想。**有时候最好的创新来自不同领域间的
   类比，而不是在同一个领域里卷得头破血流。**
