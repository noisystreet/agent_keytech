...
能帮你做出更好的工程决策——比如"这个任务用多强的模型才够"、"为什么 Agent
在处理长对话时会变笨"、"量化后的模型还能不能做复杂推理"。
...
.. admonition:: Lost in the Middle
   ...
   这意味着：
   - 窗口大了，但如果 Agent 需要的信息被淹没在大量无关上下文里
     反而会表现更差
   - 关键信息放两端，不重要信息放中间
...
.. code-block:: python
   ...
   # 缩放定律的三个实用结论
   conclusions = {
       "不要盲目追求大模型": "一个 7B 模型 + 好的提示词 + 完善的工具链，
                              可能比一个 70B 模型 + 差劲的提示词 + 没有工具链
                              更实用",
       "训练数据比模型大小更重要": "ChatGPT 的成功不只是因为模型大，
                                   更因为海量的对话数据和 RLHF 对齐",
       "针对任务选择模型": "翻译任务用 7B 可能就够，但复杂规划可能需要 70B。
                           根据实际任务的评估结果选择，而非直觉",
   }
