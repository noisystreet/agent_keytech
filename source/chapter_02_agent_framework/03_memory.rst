...
.. admonition:: 冷知识：人类工作记忆只有 7±2 个组块
   :class: funfact

   心理学经典研究发现，人类的工作记忆大约只能同时处理 7±2 个信息组块
   （George Miller, 1956）。你会记不住一长串电话号码，需要写下来。
   Agent 的短期记忆也有类似的"容量上限"——上下文窗口。不同的是，
   人类的 7±2 是硬限制，而 Agent 的窗口可以通过技术手段扩展。
   但无论窗口多大，**最关键的信息永远应该放在开头和结尾**——对人和
   对 LLM 都适用，这就是"Lost in the Middle"现象的心理学根源。
