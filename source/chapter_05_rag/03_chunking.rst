...
Agent 的上下文窗口是有限的。你不能把整本《三体》塞进一次推理——上下文
窗口再大也装不下。但你也不能把一句话切成一个块——Agent 需要看到足够的上下文才能
理解这句"不要回答！不要回答！！不要回答！！！"是在什么情景下说的。

.. admonition:: RAG 的"车祸现场"：分块切错了会怎样？
   :class: story

   一个真实的教训：某团队用 RAG 做客服问答，用户问"怎么退款"，
   Agent 回答"请拨打 110"。调查后发现：扣款政策的文档被分块时，
   刚好在"如果遇到欺诈扣款"和"请立即拨打 110"之间切开了。
   Agent 读到的块是"请立即拨打 110"，完全没看到上下文"如果遇到欺诈"。

   好的分块不只是"分段"，而是**保语义完整**。一条规则被从中间切断，
   比完全不检索更危险——因为它让 Agent 自信满满地给出错误答案。
