...
.. admonition:: 一个"Let's think"值多少钱？
   :class: funfact

   2022 年这项发现公布后，几乎一夜之间所有 AI 产品都在提示词里悄悄加上了
   "逐步推理"。OpenAI 的 ChatGPT 默认就内置了 CoT 推理链。据估算，
   光是在数学题上，CoT 就让 LLM 的可用场景扩大了 3 倍——之前模型算不对的
   小学数学题，现在能算对了。一个短语，价值数十亿美元的模型能力提升。
